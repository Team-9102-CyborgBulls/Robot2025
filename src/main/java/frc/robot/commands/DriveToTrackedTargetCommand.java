...
public class DriveToTrackedTargetCommand extends Command {
  ...
  */
  public DriveToTrackedTargetCommand(double distanceToTarget) {
    m_drivetrainSubsystem = RobotContainer.driveSubsystem;
  ...
  public void execute() {      
    ...
    if (trackedTarget != null) { // If a valid target has been retrieved
      ...
      double rotationValue = -(Math.copySign(Math.PI, rotationalError) - rotationalError) * 0.04;
      double forwardValue = -forwardError * 0.1;
      
      
      // Limit the power of the drive rate directions 
      double rotationDriveRate = limit(rotationValue, 0.6);
