...
public class TurnToTrackedTargetCommand extends Command {
  ...
  VisionSubsystem2 m_VisionSubsystem; // vision system
  ...
  public TurnToTrackedTargetCommand() {
    ...
    m_DriveSubsystem = RobotContainer.driveSubsystem; // define drive and vision system
    //m_VisionSubsystem = RobotContainer.visionSubsystem2;
    kp = 0.04; // Calculate kp scaling ratio based off scaled GYRO_KP value (used in execute())
  ...
  public void execute() {
    if (m_VisionSubsystem.getHasTarget()) { // if we find a target,
      ...
      m_DriveSubsystem.arcadeDrive(0, -1*value);
    } else {
